...
class adsethelper{
 ...
 public function createAdSetHtml(){
  ...
  $s = $s."<input type=\"hidden\" name=\"adset_type\" value=\"promotion_app_installs\">"."\n";
  $s = $s."Adset name : "."\n";
  ...
  $s = $s."Select App to promote"."\n";
  $s = $s."<select name=\"promotion_object_id\">"."\n";
  $connectionobjects = self::getConnectionObjects();
  foreach ($connectionobjects as $connectionobject){
	if ($connectionobject['type'] == 2){
		if (array_key_exists('google_play', $connectionobject['object_store_urls'])){
			$ss = $connectionobject['id'].",".$connectionobject['object_store_urls']['google_play'];
		}else{
			$ss = $connectionobject['id'];
		}
        	$s = $s. "<option value=".$ss.">".$connectionobject['name']."</option>"."\n";
	}
  }
  $s = $s."</select> <br>"."\n";
  ...
  $s = $s. "<option value=".BidTypes::BID_TYPE_ABSOLUTE_OCPM.",ACTIONS".">Mobile app installs</option>"."\n";
